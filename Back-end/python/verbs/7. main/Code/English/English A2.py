...
for count in range(8):
    urls = get_urls('English', languages['English']-1, [[]]*len(languages), 80+math.floor(count*120/8), 80+math.floor((count+1)*120/8))
    sentences = get_sentences(languages['English']-1, sentences, urls, 'Indicative Present', Tense_dict[4]['Indicative Present']-1)

    urls = get_urls('English', languages['English']-1, [[]]*len(languages), 55+math.floor(count*145/8), 55+math.floor((count+1)*145/8))
    sentences = get_sentences(languages['English']-1, sentences, urls, 'Participle Past', Tense_dict[4]['Participle Past']-1)

    urls = get_urls('English', languages['English']-1, [[]]*len(languages), math.floor(count*200/8), math.floor((count+1)*200/8))
    sentences = get_sentences(languages['English']-1, sentences, urls, 'Indicative Preterite', Tense_dict[4]['Indicative Preterite']-1)

    urls = get_urls('English', languages['English']-1, [[]]*len(languages), math.floor(count*140/8), math.floor((count+1)*140/8))
