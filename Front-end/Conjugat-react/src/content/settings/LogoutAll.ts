export const translations = {
    'English': {
        'Title1': 'Logout all',
        'Title2': 'Logged out',
        'metaContent1': '',
        'metaContent2': '',

        'Text1': 'Logout all',
        'Text2': 'Are you sure that you want to log out of all accounts, including this one?',
        'Text3': 'Logout all',
        'Text4': 'You have been successfully logged out from all devices.',
        'Text5': 'Login'
    },
    'French': {
        'Title1': 'Déconnexion de tous',
        'Title2': 'Déconnecté',
        'metaContent1': '',
        'metaContent2': '',

        'Text1': 'Déconnexion de tous',
        'Text2': 'Êtes-vous sûr de vouloir vous déconnecter de tous les comptes, y compris celui-ci ?',
        'Text3': 'Déconnexion de tous',
        'Text4': 'Vous avez été déconnecté avec succès de tous les appareils.',
        'Text5': 'Connexion'
    },
    'Italian': {
        'Title1': 'Disconnessione di tutti',
        'Title2': 'Disconnesso',
        'metaContent1': '',
        'metaContent2': '',

        'Text1': 'Disconnessione di tutti',
        'Text2': 'Siete sicuri di voler uscire da tutti gli account, compreso questo?',
        'Text3': 'Disconnessione di tutti',
        'Text4': 'Il logout da tutti i dispositivi è stato effettuato con successo.',
        'Text5': 'Accesso'
    },
    'Portuguese': {
        'Title1': 'Terminar sessão em todos',
        'Title2': 'Terminar sessão',
        'metaContent1': '',
        'metaContent2': '',

        'Text1': 'Terminar sessão em todos',
        'Text2': 'Tem a certeza de que pretende terminar a sessão de todas as contas, incluindo esta?',
        'Text3': 'Terminar sessão em todos',
        'Text4': 'A sua sessão foi terminada com sucesso em todos os dispositivos.',
        'Text5': 'Iniciar sessão'
    },
    'Spanish': {
        'Title1': 'Cerrar sesión en todas',
        'Title2': 'Logged out',
        'metaContent1': '',
        'metaContent2': '',

        'Text1': 'Cerrar sesión en todas',
        'Text2': '¿Estás seguro de que quieres cerrar la sesión de todas las cuentas, incluida ésta?',
        'Text3': 'Cerrar sesión en todas',
        'Text4': 'Se ha cerrado correctamente la sesión en todos los dispositivos.',
        'Text5': 'Inicio de sesión'
    }
}