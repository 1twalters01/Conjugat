...
for count in range(8):
    urls = get_urls('French', languages['French']-1, [[]]*len(languages), 80+math.floor(count*120/8), 80+math.floor((count+1)*120/8))
    sentences = get_sentences(languages['French']-1, sentences, urls, 'Indicatif Présent', Tense_dict[3]['Indicatif Présent']-1)

    urls = get_urls('French', languages['French']-1, [[]]*len(languages), 55+math.floor(count*145/8), 55+math.floor((count+1)*145/8))
    sentences = get_sentences(languages['French']-1, sentences, urls, 'Participe Passé', Tense_dict[3]['Participe Passé']-1)

    urls = get_urls('French', languages['French']-1, [[]]*len(languages), math.floor(count*200/8), math.floor((count+1)*200/8))
    sentences = get_sentences(languages['French']-1, sentences, urls, 'Indicatif Passé composé', Tense_dict[3]['Indicatif Passé composé']-1)

    urls = get_urls('French', languages['French']-1, [[]]*len(languages), math.floor(count*140/8), math.floor((count+1)*140/8))
