...
for count in range(8):
    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), 80+math.floor(count*120/8), 80+math.floor((count+1)*120/8))
    sentences = get_sentences(languages['Portuguese']-1, sentences, urls, 'Indicativo Presente', Tense_dict[1]['Indicativo Presente']-1)

    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), 55+math.floor(count*145/8), 55+math.floor((count+1)*145/8))
    ...
    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), math.floor(count*200/8), math.floor((count+1)*200/8))
    sentences = get_sentences(languages['Portuguese']-1, sentences, urls, 'Participio', Tense_dict[1]['Participio']-1)

    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), math.floor(count*140/8), math.floor((count+1)*140/8))
