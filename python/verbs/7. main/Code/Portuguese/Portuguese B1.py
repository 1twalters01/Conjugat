...
for count in range(12):
    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), 200+math.floor(count*250/12), 200+math.floor((count+1)*250/12))
    ...
    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), 140+math.floor(count*310/12), 140+math.floor((count+1)*310/12))
    ...
    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), math.floor(count*350/12), math.floor((count+1)*350/12))
    ...
    urls = get_urls('Portuguese', languages['Portuguese']-1, [[]]*len(languages), math.floor(count*300/12), math.floor((count+1)*300/12))
...
for index, item in enumerate(languages):
    if sentences[index]:
        f = open('python/verbs/7. main/json/'+item+' A1.json', 'w+', encoding="utf-12")
        f.write(json[index])
        f.close()