...
urls = get_urls('French', languages['French']-1, [[]]*len(languages), 0, 35)
sentences = get_sentences(languages['French']-1, sentences, urls, 'Indicatif Présent', Tense_dict[3]['Indicatif Présent']-1)

urls = get_urls('French', languages['French']-1, [[]]*len(languages), 0, 30)
sentences = get_sentences(languages['French']-1, sentences, urls, 'Participe Passé', Tense_dict[3]['Participe Passé']-1)

urls = get_urls('French', languages['French']-1, [[]]*len(languages), 35, 80)
sentences = get_sentences(languages['French']-1, sentences, urls, 'Indicatif Présent', Tense_dict[3]['Indicatif Présent']-1)

urls = get_urls('French', languages['French']-1, [[]]*len(languages), 30, 55)
sentences = get_sentences(languages['French']-1, sentences, urls, 'Participe Passé', Tense_dict[3]['Participe Passé']-1)
...
try:
    myfile = 'python/verbs/7. main/tuples/'+'English'+' A1.pkl'
    with (open(myfile, "rb")) as openfile:
        while True:
            try:
                sentences[3] = pickle.load(openfile)
            except EOFError:
                break
except:
    sentences[3] = []
